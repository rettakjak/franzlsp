...
char *symbols;			/* multiple freads mess up on 3.0 sun*/
...
lispval Lfasl()
{
	...
	char *relocs;		/* multiple freads mess up on 3.0 sun */
	...
	*/
	...
	symbols = alloca(exblk.a_syms);
	if(read(fileno(filp),symbols,exblk.a_syms)!=exblk.a_syms)
  ...
  if(debug) {
	...
	if(debug) printf(" %d symbols to relocate\n",times);
		
	relocs = alloca(exblk.a_trsize);
	if(read(fileno(filp),relocs,exblk.a_trsize)!=exblk.a_trsize)
...
#else
          for(i=0; i < times ; i++) {
  	     reloc=(struct relocation_info*)(relocs+i*sizeof(struct relocation_info));
  	     if(reloc->r_extern) {
  	         for(j=0; j < SYMMAX; j++) {
  		    if(Symbtb[j].ord == reloc->r_symbolnum) { /* look for this sym */
#define offset(p) (((p)->r_pcrel) ? ((int) code_core_org): 0)
		 ...
		 if( j >= SYMMAX) if(debug) printf("Couldnt find ord # %d\n",
						   reloc->r_symbolnum);
	     } else { /*non-external symbols weren't being relocated -> core dumps
			VAX code seemed to have bug too, since invalid symbolnums
			were being issued by the assembler for non -L options */
		if(debug) printf("Relocating local i=%d (symbolnum=%d) at %x\n",
				i,reloc->r_symbolnum,reloc->r_address);
		if(reloc->r_pcrel) error("Bad r_pcrel found",FALSE);
		*(int *)(code_core_org+reloc->r_address) += (int)code_core_org;
		    }
	}

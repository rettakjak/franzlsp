 ...
 * /\/ Make an equivalent for sstatus translink on.
 */
...
#ifndef perq_pnx		/* silent operation on Perq */
#define CL_DEBUG 1		/* otherwise, we don't mind */
#endif
...
struct bindspec *bindtab;
{
	...
	if(trans_size > 0)
	{
		...
		v = rdstr(callnames[i]);
		tranloc->name = v;
		tranloc->fcn = zlinker;	/* initially go to zlinker */
		tranloc++;
            }
	}
        else
...
/* zlinker: Transfer table link routine */

...
Lcfasl(){
	...
	work = mlbot[4].val;
	/*
	 *	BZS - should be null string not null pointer,
	 *	null pointer passed to sprintf produces "(null)" on
	 *	the 'ld' command line and the shell barks and quits.
	 *
	if(work==nil)
		largs = 0;
	 */
	...
	sprintf(cbuf,
#ifndef vax
		"%s/nld -N -x -A %s -Ttext %x %s -e %s -o %s %s -lc",
#else
